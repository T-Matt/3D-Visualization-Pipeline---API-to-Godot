...
func _ready():
	print("DEBUG - VolumeLoader starting (WORKING VERSION)...")
	load_metadata()
...
func parse_channels(binary_data: PackedByteArray) -> Dictionary:
	...
	var vals_per_channel = bytes_per_channel / 4.0
	...
	for i in range(n_channels):
		...
		for j in range(normalized_data.size()):
			...
			# Check for sentinel values and PRESERVE them
			if abs(val - (-1.0)) < 0.001:
				# Land sentinel - keep as -1.0
				processed_data[j] = -1.0
				land_count += 1
				sentinel_count += 1
			elif abs(val - (-0.5)) < 0.001:
				# Ocean sentinel - keep as -0.5  
				processed_data[j] = -0.5
				ocean_count += 1
				sentinel_count += 1
			else:
		...
		print("  Land sentinels preserved: ", land_count)
		print("  Ocean sentinels preserved: ", ocean_count)
		print("  Total sentinels: ", sentinel_count)
...
func create_texture3d(float_array: PackedFloat32Array, var_name: String) -> ImageTexture3D:
	...
	# Sample data quality during conversion
	var value_samples = []
